...
exports.isRastFinanceWorkbook = isRastFinanceWorkbook;
exports.parseRastFinanceWorkbook = parseRastFinanceWorkbook;
const import_config_1 = require("./import-config");
function isRastFinanceWorkbook(sheetNames) {
    const normalized = new Set(sheetNames.map(import_config_1.norm));
    return normalized.has((0, import_config_1.norm)("Servisler & Abonelikler"))
        && normalized.has((0, import_config_1.norm)("Ekipman Listesi"))
...
function isPaidStatus(value) {
    const status = (0, import_config_1.norm)(value);
    return status.includes("alindi") || status.includes("odendi") || status.includes("tekodeme");
}
function legacyIncomeStatuses(rows) {
    return rows.slice(2).flatMap((row) => {
        const customer = text(row[0]);
        const amount = numeric(row[1]);
        if (!customer || customer.toLocaleUpperCase("tr").startsWith("TOPLAM") || amount === undefined)
            return [];
        return [{ customer: (0, import_config_1.norm)(customer), amount, status: text(row[4]), used: false }];
    });
}
function dateFromTurkishStatus(status, year) {
    const months = {
    ...
    if (!match)
        return undefined;
    const month = months[(0, import_config_1.norm)(match[2])];
...
function parseRastFinanceWorkbook(workbook) {
    const records = [];
    const warnings = [];
    const counts = { clients: 0, invoices: 0, jobs: 0, expenses: 0, equipment: 0, plannedInstallments: 0 };
    const recurringCustomers = new Set([(0, import_config_1.norm)("Aytaş"), (0, import_config_1.norm)("Duygu Hoca"), (0, import_config_1.norm)("Newlife")]);
    ...
    const subscriptionHeader = subscriptions.findIndex((row) => (0, import_config_1.norm)(text(row[0])) === (0, import_config_1.norm)("Hizmet") && (0, import_config_1.norm)(text(row[1])).includes("usd"));
    for (let index = subscriptionHeader + 1; subscriptionHeader >= 0 && index < subscriptions.length; index++) {
        ...
        if (!service || (0, import_config_1.norm)(service).startsWith("toplam"))
            break;
    ...
    const renewalHeader = subscriptions.findIndex((row, index) => index > 10 && (0, import_config_1.norm)(text(row[0])) === (0, import_config_1.norm)("Hizmet"));
    if (renewalHeader >= 0) {
    ...
    const equipmentHeader = equipmentRows.findIndex((row) => (0, import_config_1.norm)(text(row[0])) === (0, import_config_1.norm)("Kategori") && (0, import_config_1.norm)(text(row[1])).includes("ekipmanadi"));
    for (const row of equipmentRows.slice(equipmentHeader + 1)) {
        const category = text(row[0]);
        const name = text(row[1]);
        if (!name || (0, import_config_1.norm)(category).startsWith("geneltoplam"))
            break;
        ...
        records.push({ key: "equipment", data: {
                ...
                status: (0, import_config_1.norm)(sourceStatus).includes("alinmadi") ? "planned" : "idle",
    ...
    for (const sheetName of workbook.sheetNames) {
        ...
        const detailHeader = rows.findIndex((row) => (0, import_config_1.norm)(text(row[0])) === (0, import_config_1.norm)("Tarih") && (0, import_config_1.norm)(text(row[1])).includes("musterikaynak"));
        if (detailHeader < 0)
            continue;
        for (const row of rows.slice(detailHeader + 1)) {
            ...
            if (customer && price !== undefined && price > 0) {
                const legacy = legacyStatuses.find((entry) => !entry.used && entry.customer === (0, import_config_1.norm)(customer) && entry.amount === price);
                if (legacy)
                    legacy.used = true;
                ...
                if (recurringCustomers.has((0, import_config_1.norm)(customer))) {
                    if (!createdClients.has((0, import_config_1.norm)(customer))) {
                        ...
                        createdClients.add((0, import_config_1.norm)(customer));
                        counts.clients++;
                    }
                    records.push({ key: "invoices", data: {
                            client_name: customer,
                            invoice_no: `RAST-${transactionDate.slice(0, 7)}-${(0, import_config_1.norm)(customer).toUpperCase()}`,
            ...
            if (expenseName && expenseAmount !== undefined && expenseAmount > 0) {
                ...
                const planKey = (0, import_config_1.norm)(expenseName);
    ...
    if (!usdRate)
        warnings.push("USD kuru bulunamadı; uygulamadaki mevcut kur korundu.");
