  ...
  return (
    <>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="Ödenen gider (TL)" value={TRY(total)} tone="warning" />
        <StatCard label="Tekrarlayan (aylık, TL)" value={TRY(recurring)} tone="amber" hint={`Kur: $${usd} · €${eur}`} />
        <StatCard label="Bekleyen taksit / ödeme" value={TRY(pending)} tone="danger" />
        <StatCard label="Kayıt sayısı" value={String(visibleExpenses.length)} />
      </div>
                ...
                <tr key={x.id} className="border-b border-border/60 hover:bg-surface-2/50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1">
                      <button onClick={() => { setForm({ ...x }); setOpen(true); }} className="rounded-md p-1.5 text-muted hover:bg-surface-2 hover:text-foreground"><Pencil className="h-4 w-4" /></button>
