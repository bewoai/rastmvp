...
export default function InvoicesPage() {
  ...
  const collected = hydrated
    ? visibleInvoices.reduce((sum, invoice) => sum + invoice.paid_amount, 0)
      + visibleJobs.reduce((sum, job) => sum + job.paid_amount, 0)
    : 0;
  const revenue = hydrated
    ? visibleInvoices.reduce((sum, invoice) => sum + invoice.amount + invoice.vat, 0)
  ...
  const incomeRows = [
    ...visibleInvoices.map((invoice) => ({
      ...
      client: clients.find((client) => client.id === invoice.client_id)?.name || "—",
  ...
  return (
    <>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="Toplam gelir" value={TRY(revenue)} tone="amber" />
        ...
        <StatCard label="Kayıt sayısı" value={String(hydrated ? incomeRows.length : 0)} />
      </div>
                ...
                <tr key={`${row.kind}-${row.id}`} className="border-b border-border/60 hover:bg-surface-2/50">
                  ...
                  <td className="px-4 py-3">
                    {row.kind === "recurring" && <div className="flex justify-end gap-1">
                      <button onClick={() => { setForm({ ...row.invoice }); setOpen(true); }} className="rounded-md p-1.5 text-muted hover:bg-surface-2 hover:text-foreground"><Pencil className="h-4 w-4" /></button>
