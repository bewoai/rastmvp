  ...
  return (
    <>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {sorted.map((sh) => {
          const brand = brands.find((b) => b.id === sh.brand_id);
          return (
            <div key={sh.id} className="card p-4">
              <div className="flex items-start justify-between">
                <div>
                  <p className="font-semibold text-foreground">{sh.title}</p>
                  <p className="mt-0.5 text-xs text-muted">{brand?.name || "—"} · {sh.shoot_type || "—"}</p>
                </div>
              ...
              <div className="mt-3 flex justify-end gap-1 border-t border-border pt-2">
                <button onClick={() => { setForm({ ...sh }); setOpen(true); }} className="rounded-md p-1.5 text-muted hover:bg-surface-2 hover:text-foreground"><Pencil className="h-4 w-4" /></button>
        ...
        {hydrated && shoots.length === 0 && <p className="col-span-full py-10 text-center text-muted">Henüz çekim yok.</p>}
      </div>
