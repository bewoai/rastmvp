...
export default function EquipmentManager({ view }: { view: EquipmentView }) {
  ...
  const rows = hydrated
    ? equipment.filter((item) => isPlanned ? item.status === "planned" : item.status !== "planned")
    : [];
  const inventoryValue = rows.reduce((sum, item) => sum + (item.purchase_price ?? 0), 0);

  function openNew() {
    setForm({ ...baseEmpty, status: isPlanned ? "planned" : "idle" });
    setOpen(true);
  }
                ...
                <tr key={item.id} className="border-b border-border/60 hover:bg-surface-2/50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1">
                      ...
                      <button onClick={() => { setForm({ ...item }); setOpen(true); }} className="rounded-md p-1.5 text-muted hover:bg-surface-2 hover:text-foreground"><Pencil className="h-4 w-4" /></button>
