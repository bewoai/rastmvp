...
import { ArrowUpRight, Bell, Check, ChevronDown, CircleAlert, FileText, Menu, Search, Settings, Wallet } from "lucide-react";
...
export default function Topbar({
  ...
  return (
    <header className="relative z-[30] flex h-[4.5rem] shrink-0 items-center gap-3 overflow-visible border-b border-border/70 bg-background/65 px-4 backdrop-blur-xl md:px-7">
      <button onClick={onMenu} className="rounded-xl p-2 text-muted transition-colors hover:bg-surface-2 hover:text-foreground md:hidden" aria-label="Menü">
      ...
      <div className="ml-auto flex items-center gap-2.5">
        <div ref={notificationRef} className="relative">
          <button onClick={toggleNotifications} className={`relative rounded-xl p-2.5 text-muted transition-colors hover:bg-surface-2 hover:text-foreground ${notificationOpen ? "bg-surface-2 text-foreground" : ""}`} aria-label="Bildirimler" aria-expanded={notificationOpen}>
            <Bell className="h-5 w-5" />
            {unreadCount > 0 && <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-amber" />}
          </button>
          {notificationOpen && <div className="absolute right-0 top-14 z-50 w-[min(23rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
        ...
        <div ref={accountRef} className="relative">
          <button onClick={toggleAccount} className={`flex items-center gap-2 rounded-xl p-1.5 text-left transition-colors hover:bg-surface-2 ${accountOpen ? "bg-surface-2" : ""}`} aria-label="Hesap menüsü" aria-expanded={accountOpen}>
